...
namespace atsenterapp
{
    ...
    public partial class SettingsPage : ContentPage
    {
        public SettingsPage()
        {
            ...
            autoreplySwitch.IsChecked = Global.PropertiesKeyIsNotEmpty("autoreply") && (Application.Current.Properties["autoreply"] as string) == "True";
            Entry msgTextBox = new Entry { Placeholder = "הודעת מענה אוטומטית", Text = Application.Current.Properties["autoreplytext"] as string };
            msgTextBox.TextChanged += (s, e) =>
            {
                Application.Current.Properties["autoreplytext"] = msgTextBox.Text;
                Application.Current.SavePropertiesAsync();
            };
            autoreplySwitch.AddView(msgTextBox);
        }
            ...
            sendBtn.Clicked += async (s, ea) =>
            {
                ...
                using (WebClient wc = new WebClient())
                {
                    ...
                    if (resp == "done")
                        ...
                        Global.ShowMessage("שגיאה", "נראה שיש בעיה בטופס שלך", "הבנתי");
                    else if (resp == "error")
        ...
        */
        ...
        private void LoadZmanim()
        {
            try
            {
                ...
                {
                    string[] values = new string[] { "עלות השחר א", "עלות השחר ב", "טלית ותפילין", "הנץ החמה", "ק״ש מג״א", "תפילה מג״א", "ק״ש גר״א", "תפילה גר״א", "חצות", "מנחה גדולה", "מנחה קטנה", "פלג המנחה", "שקיעה", "צאת הכוכבים" };
                    ...
                    try
                    {
                        ...
                        zmanimStr.Spans.Add(new Span { Text = "זמני היום עבור " + Global.Clock.location.Replace('_', ' ' ) + ":\n\n", FontAttributes = FontAttributes.Bold });
                    } catch { }
        ...
        {
            if (!foregroundSwitch.IsChecked && autoreplySwitch.IsChecked)
            {
                foregroundSwitch.IsChecked = true;
                Action turnOff = () =>
                {
                    foregroundSwitch.IsChecked = false;
                    autoreplySwitch.IsChecked = false;
                };
                Global.ShowQuestion("לכבות מענה אוטומטי לשיחות?", "כיבוי התכונה 'התראות צפות' יגרום לכיבוי התכונה 'מענה אוטומטי לשיחות'", "כיבוי תכונות", "ביטול", turnOff);
                return;
            }
        ...
        private void autoreplySwitch_CheckedChanged(object sender, EventArgs e)
        {
            if (autoreplySwitch.IsChecked && !foregroundSwitch.IsChecked)
            {
                autoreplySwitch.IsChecked = false;
                Action turnOn = () =>
                {
                ...
                Global.ShowQuestion("להפעיל התראות צפות?", "על מנת להפעיל את תכונת 'מענה אוטומטי לשיחות', יש להפעיל את התכונה 'התראות צפות'", "הפעל תכונות", "ביטול", turnOn);
                return;
            }
            Application.Current.Properties["autoreply"] = autoreplySwitch.IsChecked.ToString();
            Application.Current.SavePropertiesAsync();
